...
### 4.5 新手版函数链条拆解（按“按钮点击”看调用）

> 这一节只回答一个问题：你点了一个按钮，代码到底会按什么顺序跑。

#### A. 启动 App（构造函数）

1. `apps/MainApp.m` 构造函数调用 `createComponents` 创建 UI。
2. 构造函数调用 `initializeRuntimeWiring`。
3. `initializeRuntimeWiring` 依次执行：
   - `buildTemplateTree`
   - `ensureParamComponent`
   - `bindUiCallbacks`
   - `boot.startup`
4. `src/+boot/startup.m` 再依次执行：
   - `templates.getById`
   - `params.schema_get -> params.defaults -> params.validate`
   - `engine.reset`
   - `ui.applyPayload`
   - `ui.setPanelsForTemplate`
   - `ui.render`

#### B. 点击“模板树”切换模板

1. `TemplateTree.SelectionChangedFcn` 触发 `control.onTemplateChanged`。
2. `control.onTemplateChanged` 依次执行：
   - `control.parseTemplateId`
   - `templates.getById`
   - `params.schema_get/defaults/validate`
   - `templates.applyTemplatePreset`
   - `engine.reset`
   - `app.reloadParamComponent(...)`
   - `ui.setPanelsForTemplate`
   - `ui.applyPayload`
   - `ui.render`
...
1. 参数组件触发 `PayloadChanged` 事件。
2. `MainApp.ensureParamComponent` 绑定的监听会调用 `control.onParamsChanged`。
3. `control.onParamsChanged` 依次执行：
   - `ui.buildPayload`（从 UI 读值）
   - `params.validate`（归一化/裁剪/联动）
   - `ui.applyPayload`（写回统一格式）
   - `engine.reset`
   - `control.mergeRailOutputs`
   - `ui.applyOutputs`（只写输出区）
   - `ui.render`

#### D. 点击“运行”

1. `PlayButton.ButtonPushedFcn` 调用 `control.onPlay`。
2. `control.onPlay` 主路径调用 `app.startPlayback()`。
3. `app.startPlayback` 启动 `timer`，其 `TimerFcn` 绑定 `control.onTick`。
4. 每个定时器节拍进入 `control.onTick`，依次执行：
   - `advanceWithSubsteps`（内部循环调用 `engine.step`）
   - `control.mergeRailOutputs`
   - `ui.applyOutputs`
   - `ui.render`
   - `logger.logEvent`（节流日志）

#### E. 点击“重置”

1. `ResetButton.ButtonPushedFcn` 调用 `control.onReset`。
2. `control.onReset` 依次执行：
   - `app.pausePlayback`（若正在播放）
   - `engine.reset`
   - `control.mergeRailOutputs`
   - `ui.applyOutputs`
   - `ui.render`
...
1. **主业务链路没有递归**（模板切换、参数变化、运行、重置都不是递归）。
2. **播放是定时器循环，不是递归**：`timer` 周期性触发 `control.onTick`。
3. **当前已知的递归点是 UI 树搜索**：`apps/MainApp.m` 内的 `findNodeById` 用递归遍历模板树节点。

#### 为什么这点重要

1. 没有业务递归，意味着调试时调用栈通常较浅，新手更容易跟踪。
2. 有递归的 `findNodeById` 是局部工具函数，只影响“模板节点查找”，不参与物理计算。
3. 物理推进（`engine.step`）和渲染（`viz.renderScene/renderPlots`）都是普通函数调用 + 循环，不会出现递归爆栈问题。

### 4.7 关键文件总入口速查（新手常用）

1. 启动接线入口：`src/+boot/startup.m`
2. 模板切换入口：`src/+control/onTemplateChanged.m`
3. 参数变化入口：`src/+control/onParamsChanged.m`
4. 连续播放入口：`src/+control/onPlay.m`
5. 连续播放每帧：`src/+control/onTick.m`
6. 重置入口：`src/+control/onReset.m`
7. 物理重置：`src/+engine/reset.m`
8. 物理步进：`src/+engine/step.m`
9. UI 渲染总入口：`src/+ui/render.m`
10. 场景渲染：`src/+viz/renderScene.m`
11. 曲线渲染：`src/+viz/renderPlots.m`
12. 日志入口：`src/+logger/logEvent.m`
...
18. R2LC 已接入统一 R 模板（2026-02-12）：

- 不新增独立模板入口，仍使用 `R` 模板，通过 `elementType=R/C/L` 切换电阻/电容/电感分支；
- 参数 schema 扩展 `elementType/C/Ls`，并在 `params.validate` 增加 rail 联动规则；
- 引擎 `reset/step` 的 rail 分支升级为 R/C/L 三分支：
  - R：保持原阻尼解析链；
  - C：采用等效质量 `m_eff = m + C*K^2` 的闭式递推；
  - L：采用闭式旋转递推更新 `x/v/I`；
- 参数组件 `m/R2_for_test.m` 新增“回路元件”下拉框，并按元件类型动态切换参数标签（`R/C/L_s`）与输出标签（`Q/U_C/U_L`）。
